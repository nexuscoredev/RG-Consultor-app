...
export default function ContractFlowScreen() {
  ...
  const [account, setAccount] = useState('ACME Indústria Ltda.');
  const [cnpj, setCnpj] = useState('12.345.678/0001-90');
  const [service, setService] = useState('Gerenciamento de resíduos classe II');
  const [volume, setVolume] = useState('12 t/mês');
  const [value, setValue] = useState('48.000');
  ...
  const next = () => {
    if (step < STEPS.length - 1) {
      ...
      Alert.alert('Contrato registrado (demo)', 'Fluxo concluído. Na produção, isto abriria o CRM e geraria PDF.');
      setStep(0);
    }
  };
        ...
        {step === 3 ? (
          <>
            ...
            <View style={[styles.seal, { borderColor: p.goldMatte }]}>
              <FontAwesome name="leaf" size={18} color={p.goldMatte} />
