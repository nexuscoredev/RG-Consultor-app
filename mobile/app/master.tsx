...
export default function MasterDashboardScreen() {
  ...
  const totalVisits = MOCK_SELLER_TEAM.reduce((a, r) => a + r.visitsWeek, 0);
  const totalContracts = MOCK_SELLER_TEAM.reduce((a, r) => a + r.contractsMonth, 0);
  const avgXp = Math.round(MOCK_SELLER_TEAM.reduce((a, r) => a + r.xp, 0) / MOCK_SELLER_TEAM.length);
    ...
    <View style={[styles.root, { backgroundColor: p.background, paddingTop: insets.top }]}>
      <View style={[styles.topBar, { borderBottomColor: p.border }]}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.sub, { color: p.textSecondary }]}>
            {profile?.displayName ?? 'Administrador'} · {M.subtitleDemo}
          </Text>
        </View>
        <Pressable
          onPress={async () => {
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 24 }]}>
        ...
        <View style={[styles.heatmapCard, { borderColor: p.border, backgroundColor: p.card }]}>
          <Text style={[styles.heatmapHint, { color: p.textSecondary }]}>
            {M.heatHint}
          </Text>
        ...
        <Text style={[styles.section, { color: p.textSecondary }]}>Força de campo</Text>
        {MOCK_SELLER_TEAM.map((row) => (
        ...
        <View style={[styles.hintBox, { borderColor: p.border, backgroundColor: p.card }]}>
          <Text style={[styles.hintTitle, { color: p.text }]}>Painel web executivo</Text>
          <Text style={[styles.hintBody, { color: p.textSecondary }]}>
            Rode o app admin em `apps/admin` para mapa ao vivo e KPIs ampliados. Os vendedores usam este app no
            celular; o master usa e-mail `master@...` ou `admin@...` para cair nesta visão.
          </Text>
        </View>
      </ScrollView>
    </View>
